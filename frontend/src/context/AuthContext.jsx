...
// You don't necessarily need useNavigate here anymore with this revised approach
...
export const AuthProvider = ({ children }) => {
  ...
  const [token, setToken] = useState(() => {
    ...
    if (savedToken) {
      console.log("AuthContext: Found token in storage on initial load, setting header.");
  ...
  useEffect(() => {
    const interceptor = apiClient.interceptors.response.use(
      ...
      (error) => {
        console.log("Axios Interceptor: Caught error status:", error.response?.status);
  ...
  useEffect(() => {
    if (token) {
      ...
      console.log("AuthContext: Token state changed [SET], updating storage/header.");
      localStorage.setItem('authToken', token);
      apiClient.defaults.headers.common['Authorization'] = `Bearer ${token}`;
    } else {
      // If token is null (user logged out)
      console.log("AuthContext: Token state changed [NULL], removing storage/header.");
  ...
  const login = async (username, password) => {
    try {
      ...
      setToken(response.data.token); // Update state, triggers useEffects
      return { success: true };
    } catch (error) {
  ...
  const signup = async (userData) => {
     try {
       ...
       return { success: false, message: error.response?.data?.message || error.response?.data?.error || "Signup failed" };
     }
  };
